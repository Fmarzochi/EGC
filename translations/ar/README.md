...
**`egc dashboard`** - افتح متصفحك على `http://localhost:7890` وشاهد كل ما يفعله وكيل الذكاء الاصطناعي مباشرة. استدعاءات الأدوات، تعديلات الملفات، أوامر shell، استخدام الرموز، حالة الذاكرة وحالة الوكيل في الوقت الفعلي. تبدأ لوحة التحكم تلقائياً بعد `egc init`.

```
egc dashboard          # تشغيل لوحة التحكم (تبدأ تلقائياً بعد egc init)
egc dashboard stop     # إيقاف الخادم في الخلفية
egc dashboard status   # التحقق من حالة تشغيل الخادم
```
