...
This loads the EGC OpenCode plugin module from npm:
...
After installation, the `egc-install` CLI is also available:
...
- `EGC_HOOK_PROFILE`: `minimal`, `standard` (default), `strict`
- `EGC_DISABLED_HOOKS`: comma-separated hook IDs to disable

## Skills

The default OpenCode config loads 11 curated EGC skills via the `instructions` array:
