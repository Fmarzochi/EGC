...
| 2 | **Antigravity (AGY)** | 1 | `antigravity` | `.agents/` (project-scoped, per repo) | Skills, agents, rules, and commands installed per-project; GateGuard hooks registered; plus the home-level assets the `egc` target owns under `~/.gemini/antigravity-cli/` |
| 3 | **Cursor** | 1 | `cursor` | `.cursor/` (project-scoped, per repo) | Rules as `.cursor/rules/*.mdc`, the Guardian and Token Crusher hooks in `.cursor/hooks.json`, and the servers of the project `.mcp.json` merged into `.cursor/mcp.json`, all per project; the user-level `~/.cursor/mcp.json` registration is done by `egc init` and the shell installers through the shared MCP list, not by this target |
