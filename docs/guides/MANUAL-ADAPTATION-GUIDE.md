...
Use this guide when you want egc behavior inside a harness that does not natively load `.gemini/`, `.codex/`, `.opencode/`, `.cursor/`, or `.agent/` layouts.
...
Use this guide only when you need egc behavior in a non-native harness.

## What You Are Reproducing

When you adapt egc manually, you are trying to preserve four things:
...
Optional: if you already use a repo packer like `repomix`, it can help compress selected files into one handoff document. It is a convenience tool, not the canonical egc path.

## Compression Rules

When manually packing egc for another harness:
...
You are not implementing real commands. You are giving the harness explicit invocation handles that map to egc behavior.
...
That does not recreate true automation, but it captures the operational discipline of egc.

## Harness Capability Matrix

| Capability | First-Class egc Targets | Manual-Adaptation Targets |
| --- | --- | --- |
...
2. Pack the selected egc skill files into one upload or paste block.
...
```text
You are operating with a manually adapted egc bundle.
...
- use manual adaptation to carry egc behavior into non-native harnesses
- use native egc targets whenever you want the full system
