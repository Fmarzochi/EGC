...
- A security hardening round lands as eighteen steps (#1356 to #1363, #1365 to #1373): the memory server, the installer, the hooks and the dashboard get tighter defaults and stricter checks on what they accept from disk, from the network and from the tools around them, each step with its own tests and its own review. Nothing changes in the documented commands; all of it ships with the next release.
