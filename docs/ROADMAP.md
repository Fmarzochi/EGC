...
## v1.1.22: The Compact Install Check (Released 2026-09-05)
- `egc init` ends with a compact install check instead of the full doctor report: one line when healthy, only the targets that need attention with the exact command to run when not (`egc repair` for drift and version skew, the full-profile install for what the automatic repair could not fix), a live spinner on an interactive terminal and a plain line without one. The memory and Token Crusher lines move above the check and report real state instead of a fixed sentence, the closing tip and the compression claim are gone, the dashboard is one check line that respects the headless decision, and `Installation complete.` is the last line. `egc doctor` keeps its full report and its JSON contract (#1384)
