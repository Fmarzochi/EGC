...
- The EGC Scrubber lands its first layer: an automatic, deterministic hygiene pass for content you own. It strips invisible Unicode carriers (zero-width family, illegitimate bidi, tag characters, variation selectors, private-use, soft hyphen) and normalizes long dashes into safe punctuation, while preserving the invisibles that carry meaning (emoji joiners, complex-script joiners, complete flag tag sequences, same-script fillers, RTL marks, paired bidi embeddings). It adds a Write/Edit hook that cleans content before it reaches disk, a commit hook that removes AI co-authorship while keeping human co-authors, a manual `content-scrubber` skill and CLI, and a spec. Pure Node so it runs the same on Linux, Windows, and macOS; additive with no install-engine changes; and fail-open, so a parse error, an engine error, or binary input passes the original content through untouched and can never block or corrupt a write. The deterministic layer is verifiable by count; statistical token-sampling marks stay out of scope for the automatic path and are handled best-effort by a documented rewrite workflow, never claimed undetectable (#1301)
