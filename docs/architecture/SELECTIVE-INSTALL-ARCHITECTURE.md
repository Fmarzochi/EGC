...
- what egc owns and may safely remove or repair later

That is the missing contract between egc 1.x installation and an egc 2.0
control plane.
...
- unified `egc` CLI routing install, planning, and lifecycle commands
...
- legacy `egc-install` compatibility still points at `install.sh`
...
- the repo already has a unified Node entrypoint through `egc` and
  `install-apply.js`
...
- gemini may preserve native hierarchy under `~/.gemini`
...
egc 2.0 should make operation planning a standalone phase so that:
...
- `uninstall`: remove only egc-owned outputs
...
- `scripts/egc.js` as the unified CLI
...
If the goal is egc 2.0 and not just “working enough,” the next modularization
steps should be:
...
Today egc still behaves like a broad payload copier:
...
That means egc 2.0 needs two contracts, not one:

- a content contract
  what modules exist and how they depend on each other
- a target contract
  how those modules land inside gemini, Cursor, Antigravity, Codex, or OpenCode
...
1. Keep `EGC` as the canonical source repo.
...
Fields still needed for egc 2.0:

- `installStrategy`
  for example `copy`, `flatten-rules`, `generate`, `merge-config`
- `ownership`
  whether egc fully owns the target path or only generated files under it
- `pathMode`
...
That lets egc 2.0 say things like:

- `developer` is the recommended default for gemini and Cursor
...
egc 2.0 needs the next layer: operation planning.
...
Install-state is the durable contract that egc 1.x is missing.

Suggested path conventions:

- gemini target:
  `~/.gemini/egc/install-state.json`
- Cursor target:
  `./.cursor/egc-install-state.json`
- Antigravity target:
  `./.agent/egc-install-state.json`
- future Codex target:
  `~/.codex/egc-install-state.json`
...
- enough detail for uninstall to remove only egc-managed outputs
...
- `egc list-installed` routes to `node scripts/list-installed.js`
- `egc uninstall` routes to `node scripts/uninstall.js`
- `egc doctor` routes to `node scripts/doctor.js`
- `egc repair` routes to `node scripts/repair.js`
...
- load install-state
- remove only egc-managed destinations recorded in state
...
egc 2.0 should translate legacy language arguments into a compatibility request.
...
egc 2.0 should improve this carefully.
...
3. define `egc.install.v1` state schema
4. write install-state on successful install

### Phase 2: Target Adapters

1. extract gemini install behavior into `gemini-home` adapter
...
4. decide when `egc-install` should become a thin alias for `egc install`
...
3. reduce package shipping surface to the module graph instead of broad folders
4. decide whether `egc-install` remains separate or becomes `egc install`
5. add tests that lock down:
   - target-specific merge/remove behavior
   - repair and uninstall safety for non-copy operations
   - unified `egc` CLI routing and compatibility guarantees
...
5. Should Codex and OpenCode target adapters ship only after the gemini/Cursor
...
That is the shortest path from egc 1.x installer sprawl to an egc 2.0
