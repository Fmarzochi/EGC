...
- Guardian validation depends on the harness actually invoking the registered hooks. Amazon Q, Goose, and OpenHands (#1092) and Qwen Code (#1080) all have real Guardian hook wiring. Roo Code was the one exception until its retirement on 2026-08-16, and this paragraph describes how it worked while it was installed: it has no external hook API (confirmed against its own docs and an open upstream feature request), so it gets a native `roo-cline.deniedCommands` seed instead of a Guardian adapter, a real but partial mitigation covering only the unconditionally-dangerous base commands by prefix match, not context-aware command-level enforcement.
