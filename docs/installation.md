...
Each of those steps is one check line: `cognitive protocol` says how many tools carry the current protocol and names the ones it installed or upgraded, `MCP registration` says how many tools already had the servers and names the ones it wrote, `memory` reports the state store with its migration count, and `commit-privacy filter` lists what it changed in the repository the first time and reports the filter as configured afterwards. A detail line appears only when something changed, was skipped or failed.
...
`egc install --prompt-library` does the same for every detected tool at once, without the question: every home target whose config directory or command is present, and Trae and CodeBuddy under the home directory (their project adapters run with the home as the project, the way their former scripts did with `~`). Claude Code receives the agents in `~/.claude/agents`, the commands in `~/.claude/commands` and the rules in `~/.claude/rules`, all in the format Claude Code reads.
...
Task routing stays on your machine and needs no API key. On every prompt, the router scores the local catalog (name, description and the situations each skill names), shortlists only what the active tool has installed, and leaves the decision to the model of your tool, by intent, in the prompt's own language. Components that exist in the catalog but are not installed for that tool are named as such, with the command that adds them, and never offered as invocable. `orchestrate_task` follows the same rule and lists them under `not_installed`. Sending the task prompt to a provider for semantic routing is an opt-in on top of that, one variable per surface: `EGC_ROUTING_MODE=llm` for the prompt router and `EGC_LLM_ROUTING=1` for `orchestrate_task`, each in addition to a provider key; without the opt-in a configured key changes nothing about routing.
...
Populated memory never reaches a commit. The propagation files (AGENTS.md, GEMINI.md, editor rules) ship as empty structure; local sessions repopulate them, a pre-commit hook blocks accidental staging, and a CI guard catches anything that slips past local hooks. Since v1.1.13 `egc init` adds a third local layer: a git clean filter (`filter.egc-memory.clean`) bound to the propagation files in `.git/info/attributes`, so `git add` stages a zeroed blob even when hooks are bypassed with `--no-verify`. The filter configuration stays entirely inside `.git` (nothing tracked is modified), the working tree keeps the populated memory, and the installer prints the exact actions before applying them, honoring `--dry-run`. The context files receive the memory block only after that filter is in place: in a checkout where it cannot be set up (git cannot open it, the filter script is not on disk, the local configuration cannot be written), the files are left as they are, one line on stderr says why and points at `egc doctor`, and the memory itself stays in `~/.egc/state`. Outside a repository nothing changes. The smudge side of the same filter puts the memory back: whenever git writes one of those files anew (a pull that touched it, a branch switch, a stash pop), the block is rebuilt from `~/.egc/state`, so the working tree never loses it and the file reads as unmodified afterwards; where the state cannot be read, or the script is gone, the file comes out as committed and the checkout goes on. The clean side zeroes the llms.txt form of the block as well.
