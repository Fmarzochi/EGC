...
Prefer dry-run plans and local queue files before enabling recurring or event-driven actions. Keep credentials, private workspace exports, personal datasets, and account-specific automations out of reusable EGC artifacts.
...
# Via egc -p (programmatic mode)
echo "Review open PRs and summarize" | egc -p --project /path/to/repo
```
...
egc -p "Analyze the output of the security scan and create issues for findings"
```
...
| Context Manager | Session management + memory | EGC 2.0 session lifecycle |
...
egc -p "Create a scheduled task: every weekday at 9am, review my GitHub notifications, open PRs, and calendar. Write a morning briefing to memory."

# Continuous learning
egc -p "Create a scheduled task: every Sunday at 8pm, extract patterns from this week's sessions and update the learned skills."
```
...
egc -p "Create memory entities for: me (user profile), my projects, my key contacts. Add observations about current priorities."
```
