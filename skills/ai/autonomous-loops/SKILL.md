...
egc -p "Read the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. Write tests first (TDD). Do NOT create any new documentation files."

# Step 2: De-sloppify (cleanup pass)
egc -p "Review all files changed by the previous commit. Remove any unnecessary type tests, overly defensive checks, or testing of language features (e.g., testing that TypeScript generics work). Keep real business logic tests. Run the test suite after cleanup."

# Step 3: Verify
egc -p "Run the full build, lint, type check, and test suite. Fix any failures. Do not add new features."

# Step 4: Commit
egc -p "Create a conventional commit for all staged changes. Use 'feat: add OAuth2 login flow' as the message."
```

### Key Design Principles

1. **Each step is isolated**: A fresh context window per `egc -p` call means no context bleed between steps.
...
egc -p --model gemini-2.5-pro "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Sonnet (fast, capable)
egc -p "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Opus (thorough)
egc -p --model gemini-2.5-pro "Review all changes for security issues, race conditions, and edge cases..."
```
...
egc -p "Read .gemini-context.md for priorities. Work through them in order."
...
egc -p --allowedTools "Read,Grep,Glob" "Audit this codebase for security vulnerabilities..."

# Write-only implementation pass
egc -p --allowedTools "Read,Write,Edit,Bash" "Implement the fixes from security-audit.md..."
```
...
**EGC's built-in persistent loop.** A session-aware REPL that calls `egc -p` synchronously with full conversation history.
...
2. Each user message is sent to `egc -p` with full history as context
...
│                                                     │
│  1. Create branch (continuous-egc/iteration-N)   │
│  2. Run egc -p with enhanced prompt              │
│  3. (Optional) Reviewer pass: separate egc -p   │
│  4. Commit changes (egc generates message)       │
│  5. Push + create PR (gh pr create)                 │
│  6. Wait for CI checks (poll gh pr checks)          │
│  7. CI failure? → Auto-fix pass (egc -p)         │
...
> **Warning:** Install continuous-egc from its repository after reviewing the code. Do not pipe external scripts directly to bash.

### Usage

```bash
# Basic: 10 iterations
continuous-egc --prompt "Add unit tests for all untested functions" --max-runs 10

# Cost-limited
continuous-egc --prompt "Fix all linter errors" --max-cost 5.00

# Time-boxed
continuous-egc --prompt "Improve test coverage" --max-duration 8h
...
continuous-egc --prompt "Add tests" --max-runs 5 --worktree tests-worker &
continuous-egc --prompt "Refactor code" --max-runs 5 --worktree refactor-worker &
wait
```
...
Gemini reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `egc -p` invocations.

### CI Failure Recovery

When PR checks fail, Continuous Gemini automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `egc -p` with CI fix context
...
egc -p "Implement the feature with full TDD. Be thorough with tests."

# Step 2: De-sloppify (separate context, focused cleanup)
egc -p "Review all changes in the working tree. Remove:
...
for feature in "${features[@]}"; do
  # Implement
  egc -p "Implement $feature with TDD."

  # De-sloppify
  egc -p "Cleanup pass: review changes, remove test/code slop, run tests."

  # Verify
  egc -p "Run build + lint + tests. Fix any failures."

  # Commit
  egc -p "Commit with message: feat: add $feature"
done
```
...
| Multi-day project | Yes | Maybe (continuous-egc) |
...
3. **Any loop + Verification**: Use EGC's `/verify` command or `verification-loop` skill as a gate before commits.
...
4. **Ralphinho's tiered approach in simpler loops**: Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ...
   egc -p --model haiku "Fix the import ordering in src/utils.ts"

   # Complex architectural change
   egc -p --model gemini-2.5-pro "Refactor the auth module to use the strategy pattern"
   ```
...
2. **No context bridge between iterations**: Each `egc -p` call starts fresh. Use `SHARED_TASK_NOTES.md` or filesystem state to bridge context.
