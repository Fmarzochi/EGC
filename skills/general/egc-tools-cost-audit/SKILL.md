...
# EGC Tools Cost Audit

Use this skill when the user suspects the EGC Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [EGC-Tools](../../EGC-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.
...
- user says EGC Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `EGC-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `EGC-Tools` repo, not in `EGC`
...
- switch into the sibling `EGC-Tools` repo
