...
- Quick single-file fixes (use standard `egc -p`)
...
```bash
# Step 1: Plan
egc -p --model gemini-2.5-pro "You are a Product Planner. Read PLANNER_PROMPT.md. Expand this brief into a full product spec: 'Build a Kanban board app'. Write spec to spec.md"

# Step 2: Generate (iteration 1)
egc -p --model gemini-2.5-pro "You are a Generator. Read spec.md. Implement Sprint 1. Start the dev server on port 3000."

# Step 3: Evaluate (iteration 1)
egc -p --model gemini-2.5-pro --allowedTools "Read,Bash,mcp__playwright__*" "You are an Evaluator. Read EVALUATOR_PROMPT.md. Test the live app at http://localhost:3000. Score against the rubric. Write feedback to feedback-001.md"

# Step 4: Generate (iteration 2: reads feedback)
egc -p --model gemini-2.5-pro "You are a Generator. Read spec.md and feedback-001.md. Address all issues. Improve the scores."
...
| `GAN_PLANNER_MODEL` | `gemini-2.5-pro` | Model for planning agent |
| `GAN_GENERATOR_MODEL` | `gemini-2.5-pro` | Model for generator agent |
| `GAN_EVALUATOR_MODEL` | `gemini-2.5-pro` | Model for evaluator agent |
