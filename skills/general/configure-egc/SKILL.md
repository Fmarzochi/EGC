...
- User says "configure egc", "install egc", "setup everything egc code", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing EGC installation
...
cp -R "$EGC_ROOT/.agents/skills/<skill-name>" "$TARGET/skills/"

# Niche skills live under skills/
cp -R "$EGC_ROOT/skills/<skill-name>" "$TARGET/skills/"
```
...
cp -r $EGC_ROOT/rules/common/* $TARGET/rules/

# Language-specific rules (flat copy into rules/)
cp -r $EGC_ROOT/rules/typescript/* $TARGET/rules/   # if selected
cp -r $EGC_ROOT/rules/python/* $TARGET/rules/        # if selected
cp -r $EGC_ROOT/rules/golang/* $TARGET/rules/        # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source EGC repository (`$EGC_ROOT/`).
