...
├─ Spawns egc -p subprocess with violations JSON
...
| Model routing | Manual (`/model gemini-2.5-pro`) | Automatic (violation complexity → tier) |

### Recommended Combination

1. Install EGC as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use EGC's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both EGC and Plankton hooks:
- EGC's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable EGC's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (EGC handles what Plankton doesn't cover)
...
```json
{
  ...
  "subprocess": {
    "tiers": {
      ...
      "gemini-2.5-pro":   { "timeout": 600, "max_turns": 15 }
    },
    "volume_threshold": 5
  }
}
