 ...
 * directory with the root TypeScript dependency, and uses the same sqlite / sqlite3
...
const SESSION_BUS_SOURCE = path.join(REPO_ROOT, 'mcp', 'servers', 'egc-memory', 'src', 'session-bus.ts');
...
function compileProductionBus(tempDir) {
...
async function runHarness() {
  ...
  const busModule = compileProductionBus(tempRoot);
