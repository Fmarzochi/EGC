...
function runTests() {
  ...
  const IS_A_COMMAND = [
    ...
    ['sudo -Hu root rm -rf /tmp/x', 'a wrapper with grouped flags ending in a value'],
    ['sudo --us root rm -rf /tmp/x', 'an abbreviated wrapper option'],
    ['timeout -vk 9 5 rm -rf /tmp/x', 'a grouped value flag and a duration'],
    ['env -u HOME rm -rf /tmp/x', 'an env option taking a value'],
    ['nice git reset --hard', 'nice, which takes no positional'],
    ['ionice -c 3 git reset --hard', 'ionice with its class'],
  ...
  if (test('uses project directory fallback when no session or transcript id exists', () => {
    ...
    const fallbackEnv = {
      ...
      ECC_SESSION_ID: '',
      GEMINI_TRANSCRIPT_PATH: '',
      GEMINI_PROJECT_DIR: path.join(stateDir, 'project-root'),
    };
