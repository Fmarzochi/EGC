...
async function runTests() {
  ...
  if (
    await asyncTest('matches getSessionIdShort when transcript UUID equals EGC_SESSION_ID', async () => {
      ...
      try {
        ...
        assert.ok(fs.existsSync(sessionFile), `Session filename should match the pre-fix EGC_SESSION_ID-based name: ${sessionFile}`);
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('respects GEMINI_PACKAGE_MANAGER for formatter fallback runner', async () => {
      ...
      const stdinJson = JSON.stringify({ tool_input: { file_path: filePath } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-format.js'), stdinJson, withPrependedPath(binDir, { GEMINI_PACKAGE_MANAGER: 'pnpm' }));
  ...
  if (
    await asyncTest('respects project package-manager config for formatter fallback runner', async () => {
      ...
      fs.mkdirSync(path.join(rootDir, '.gemini'), { recursive: true });
      fs.writeFileSync(path.join(rootDir, '.gemini', 'package-manager.json'), JSON.stringify({ packageManager: 'bun' }));
      fs.writeFileSync(path.join(rootDir, '.prettierrc'), '{}');
      fs.writeFileSync(filePath, 'export const value = 1;\n');
  ...
  if (
    test('SessionStart hook uses safe inline resolver without plugin-tree scanning', () => {
      ...
      assert.ok(commandText.includes('GEMINI_PLUGIN_ROOT'), 'SessionStart should use GEMINI_PLUGIN_ROOT');
      assert.ok(!commandText.includes('${GEMINI_PLUGIN_ROOT}'), 'SessionStart should not depend on raw shell placeholder expansion');
  ...
  if (
    test('Stop and SessionEnd hooks use the safe inline resolver when plugin root may be unset', () => {
      ...
      for (const hook of [...stopHooks, ...sessionEndHooks]) {
        ...
        assert.ok(commandText.includes('GEMINI_PLUGIN_ROOT'), 'Lifecycle hook should consult GEMINI_PLUGIN_ROOT');
        assert.ok(!commandText.includes('${GEMINI_PLUGIN_ROOT}'), 'Lifecycle hook should not depend on raw shell placeholder expansion');
        ...
        for (const entry of hookArray) {
          for (const hook of entry.hooks) {
            ...
            if (hook.type === 'command' && commandText.includes('scripts/hooks/')) {
              ...
              assert.ok(!commandText.includes('${GEMINI_PLUGIN_ROOT}'), `Script paths should not depend on raw shell placeholder expansion: ${commandText.substring(0, 80)}...`);
              assert.ok(
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      // See: https://github.com/Fmarzochi/everything-gemini/issues/103
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('ECC_OBSERVER_MAX_TURNS'), 'observer-loop should keep the legacy max-turn override');
      assert.ok(observerLoopSource.includes('${ECC_OBSERVER_MAX_TURNS:-20}'), 'observer-loop should default to 20 turns via the legacy fallback chain');
      assert.ok(!observerLoopSource.includes('--max-turns 3'), 'observer-loop should not hardcode a 3-turn limit');
      assert.ok(observerLoopSource.includes('EGC_SKIP_OBSERVE=1'), 'observer-loop should suppress observe.sh for automated sessions (canonical)');
      assert.ok(observerLoopSource.includes('ECC_SKIP_OBSERVE=1'), 'observer-loop should keep the legacy observe-suppression guard');
      assert.ok(observerLoopSource.includes('EGC_HOOK_PROFILE=minimal'), 'observer-loop should run automated analysis with the minimal hook profile (canonical)');
      assert.ok(observerLoopSource.includes('ECC_HOOK_PROFILE=minimal'), 'observer-loop should keep the legacy minimal-profile guard');
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('detect-project writes project metadata to the registry and project directory', async () => {
      ...
      try {
        ...
        assert.strictEqual(metadata.remote, 'https://github.com/example/egc-test.git', 'project.json should include the sanitized remote');
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips configured observer-session paths (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips configured observer-session paths before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        name: 'cwd skip path',
        env: {
          GEMINI_CODE_ENTRYPOINT: 'cli',
          ECC_OBSERVE_SKIP_PATHS: ' observer-sessions , .gemini-mem '
        },
  ...
  if (
    await asyncTest('updates Last Updated timestamp in existing session file', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'session-end.js'), '', {
        HOME: testDir,
        USERPROFILE: testDir,
        EGC_SESSION_ID: `session-${shortId}`
      });
      assert.strictEqual(result.code, 0);
    ...
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      ...
      try {
        const result = await new Promise((resolve, reject) => {
          ...
          delete env.GEMINI_PACKAGE_MANAGER; // Remove any env-level PM override
