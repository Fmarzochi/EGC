...
function runTests() {
  ...
  if (test('parses manifest-mode CLI arguments', () => {
    ...
    assert.strictEqual(parsed.configPath, 'egc-install.json');
  ...
  if (test('normalizes legacy language installs into a canonical request', () => {
    ...
    assert.strictEqual(request.target, 'egc');
  ...
  if (test('merges config-backed component selections with CLI overrides', () => {
    const request = normalizeInstallRequest({
      ...
      excludeComponentIds: ['capability:media'],
      languages: [],
      configPath: '/workspace/app/egc-install.json',
      config: {
        path: '/workspace/app/egc-install.json',
        target: 'egc',
        profileId: 'developer',
    ...
    assert.strictEqual(request.configPath, '/workspace/app/egc-install.json');
  })) passed++; else failed++;
