...
**Started:** 10:30
**Last Updated:** 14:45
**Project:** everything-gemini
**Branch:** feature/session-metadata
**Worktree:** /tmp/egc-worktree
...
\`\`\``;
    ...
    assert.strictEqual(meta.project, 'everything-gemini');
    assert.strictEqual(meta.branch, 'feature/session-metadata');
    assert.strictEqual(meta.worktree, '/tmp/egc-worktree');
...
`;
  ...
  const tmpHome = path.join(os.tmpdir(), `egc-session-mgr-test-${Date.now()}`);
  const tmpCanonicalSessionsDir = path.join(tmpHome, '.gemini', 'session-data');
  const tmpLegacySessionsDir = path.join(tmpHome, '.gemini', 'sessions');
  ...
  const r33Home = path.join(os.tmpdir(), `egc-r33-birthtime-${Date.now()}`);
  const r33SessionsDir = path.join(r33Home, '.gemini', 'sessions');
  ...
  if (test('getAllSessions returns empty when sessions dir missing', () => {
    ...
    const origUserProfile = process.env.USERPROFILE;
    try {
      // Point HOME to a dir with no .gemini/sessions/
  ...
  if (test('getAllSessions skips subdirectories inside sessions dir', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `egc-r89-subdir-skip-${Date.now()}`);
    const sessionsDir = path.join(isoHome, '.gemini', 'sessions');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  const tmpSessionsDir2 = path.join(tmpHome2, '.gemini', 'sessions');
...
`;
  ...
  if (test('getSessionById matches old format YYYY-MM-DD-session.tmp via noIdMatch path', () => {
    ...
    try {
      ...
      if (origDir) process.env.GEMINI_DIR = origDir;
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      delete require.cache[require.resolve('../../scripts/lib/session-manager')];
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  if (test('getAllSessions date filter uses strict equality so wrong format returns empty', () => {
    ...
    fs.mkdirSync(path.join(homeDir, '.gemini', 'sessions'), { recursive: true });
