...
function runTests() {
  ...
  if (test('validates install component filters', () => {
    ...
    assert.ok(claudeComponents.every(component => component.targets.includes('egc')));
  ...
  if (test('skips a requested module when its dependency chain does not support the target', () => {
    ...
    try {
      ...
      const plan = resolveInstallPlan({ repoRoot, profileId: 'core', target: 'egc' });
  ...
  if (test('fails fast when install manifest module targets is not an array', () => {
    ...
    try {
      ...
      assert.throws(
        () => resolveInstallPlan({ repoRoot, profileId: 'core', target: 'egc' }),
