...
test('buildOrchestrationPlan creates worktrees, branches, and tmux commands', () => {
  const repoRoot = path.join('/tmp', 'egc');
  ...
  assert.ok(
    plan.workerPlans[0].worktreePath.endsWith(path.join('egc-skill-audit-docs-a')),
...
test('buildOrchestrationPlan normalizes global and worker seed paths', () => {
  const plan = buildOrchestrationPlan({
    ...
    seedPaths: ['scripts/orchestrate-worktrees.js', './.gemini/plan/workflow-e2e-test.json'],
    workers: [
      {
        name: 'Docs',
        task: 'Update docs',
...
test('normalizeSeedPaths rejects paths outside the repo root', () => {
  assert.throws(
    () => normalizeSeedPaths(['../outside.txt'], '/tmp/egc'),
    /inside repoRoot/
  );
});

test('materializePlan keeps worker instructions inside the worktree boundary', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-orchestrator-test-'));

  try {
    const plan = buildOrchestrationPlan({
      repoRoot: tempRoot,
      coordinationRoot: path.join(tempRoot, '.gemini', 'orchestration'),
...
test('overlaySeedPaths copies local overlays into the worker worktree', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-orchestrator-overlay-'));
  const repoRoot = path.join(tempRoot, 'repo');
  const worktreePath = path.join(tempRoot, 'worktree');

  try {
    fs.mkdirSync(path.join(repoRoot, 'scripts'), { recursive: true });
    fs.mkdirSync(path.join(repoRoot, '.gemini', 'plan'), { recursive: true });
    ...
    fs.writeFileSync(
      path.join(repoRoot, '.gemini', 'plan', 'workflow-e2e-test.json'),
      '{"seeded":true}\n',
      'utf8'
    );
    fs.writeFileSync(
    ...
    assert.strictEqual(
      fs.readFileSync(path.join(worktreePath, '.gemini', 'plan', 'workflow-e2e-test.json'), 'utf8'),
      '{"seeded":true}\n'
    );
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true });
...
test('executePlan rolls back partial setup when orchestration fails mid-run', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/egc/.orchestration/rollback-test',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/egc-rollback-docs',
        seedPaths: ['commands/multi-workflow.md'],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-test-docs', '/tmp/egc-rollback-docs', 'HEAD'],
...
test('executePlan does not mark pre-existing resources for rollback when worktree creation fails', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/egc/.orchestration/rollback-existing',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/egc-existing-docs',
        seedPaths: [],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-existing-docs', '/tmp/egc-existing-docs', 'HEAD'],
        launchCommand: 'echo run',
