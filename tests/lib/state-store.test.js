...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    ...
    try {
      ...
      assert.strictEqual(firstMigrations.length, 6);
      ...
      assert.strictEqual(firstMigrations[5].version, 6);
      ...
      assert.strictEqual(secondMigrations.length, 6);
      ...
      assert.strictEqual(secondMigrations[5].version, 6);
    } finally {
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    ...
    try {
      ...
      assert.strictEqual(store.getAppliedMigrations().length, 6);
      store.close();
