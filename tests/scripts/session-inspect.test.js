...
function runTests() {
  ...
  if (test('lists registered adapters', () => {
    ...
    assert.ok(payload.adapters.some(adapter => adapter.id === 'egc-history'));
    assert.ok(payload.adapters.some(adapter => adapter.id === 'dmux-tmux'));
  })) passed++; else failed++;

  if (test('prints canonical JSON for egc history targets', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-inspect-home-'));
    const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-inspect-recordings-'));
    ...
    try {
      ...
      assert.strictEqual(persisted.latest.adapterId, 'egc-history');
  ...
  if (test('supports explicit target types for structured registry routing', () => {
    ...
    try {
      ...
      const result = run(['latest', '--target-type', 'egc-history'], {
        env: { HOME: homeDir }
      });
      ...
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.adapterId, 'egc-history');
      assert.strictEqual(payload.session.sourceTarget.type, 'egc-history');
  ...
  if (test('writes snapshot JSON to disk when --write is provided', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-inspect-home-'));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-inspect-out-'));
    const sessionsDir = path.join(homeDir, '.gemini', 'sessions');
    fs.mkdirSync(sessionsDir, { recursive: true });
    ...
    try {
      ...
      const result = run(['egc:latest', '--write', outputPath], {
        env: { HOME: homeDir }
      });
      ...
      assert.strictEqual(written.adapterId, 'egc-history');
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('inspects skill health from recorded observations', () => {
    ...
    try {
      ...
      assert.strictEqual(result.code, 0, result.stderr);
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'egc.skill-health.v1');
  ...
  if (test('proposes skill amendments through session-inspect', () => {
    ...
    try {
      ...
      assert.strictEqual(result.code, 0, result.stderr);
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'egc.skill-amendment-proposal.v1');
  ...
  if (test('builds skill evaluation scaffolding through session-inspect', () => {
    ...
    try {
      ...
      assert.strictEqual(result.code, 0, result.stderr);
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'egc.skill-evaluation.v1');
