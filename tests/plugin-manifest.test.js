...
const opencodeHooksPluginPath = path.join(repoRoot, '.opencode', 'plugins', 'egc-hooks.ts');
...
// ── Gemini plugin manifest ────────────────────────────────────────────────────
console.log('\n=== .gemini-plugin/plugin.json ===\n');

const claudePluginPath = path.join(repoRoot, '.gemini-plugin', 'plugin.json');
const claudeMarketplacePath = path.join(repoRoot, '.gemini-plugin', 'marketplace.json');

test('egc plugin.json exists', () => {
  assert.ok(fs.existsSync(claudePluginPath), 'Expected .gemini-plugin/plugin.json to exist');
});

const claudePlugin = loadJsonObject(claudePluginPath, '.gemini-plugin/plugin.json');

test('egc plugin.json has version field', () => {
  assert.ok(claudePlugin.version, 'Expected version field');
});

test('egc plugin.json version matches package.json', () => {
...
test('egc plugin.json skills is an array', () => {
  assert.ok(Array.isArray(claudePlugin.skills), 'Expected skills to be an array');
});

test('egc plugin.json commands is an array', () => {
...
test('egc plugin.json disables bundled MCP servers for provider tool-name compatibility', () => {
  ...
  assert.deepStrictEqual(
    ...
    {},
    'Gemini plugin installs must not auto-bundle root MCP servers; document/manual MCP install remains supported',
  );
});
...
test('egc marketplace.json exists', () => {
  assert.ok(fs.existsSync(claudeMarketplacePath), 'Expected .gemini-plugin/marketplace.json to exist');
});

const claudeMarketplace = loadJsonObject(claudeMarketplacePath, '.gemini-plugin/marketplace.json');

test('egc marketplace.json keeps only Gemini-supported top-level keys', () => {
  const unsupportedTopLevelKeys = ['$schema', 'description'];
  for (const key of unsupportedTopLevelKeys) {
    assert.ok(
      !(key in claudeMarketplace),
      `.gemini-plugin/marketplace.json must not declare unsupported top-level key "${key}"`,
    );
  }
});
...
test('codex plugin.json uses short plugin slug', () => {
  assert.strictEqual(codexPlugin.name, 'egc');
});
...
test('marketplace.json uses short marketplace slug', () => {
  assert.strictEqual(marketplace.name, 'egc');
});
...
test('marketplace.json plugin entry uses short plugin slug', () => {
  assert.strictEqual(marketplace.plugins[0].name, 'egc');
});
...
test('user-facing docs do not use deprecated egc@egc install commands', () => {
  ...
  for (const filePath of markdownFiles) {
    ...
    if (/\/plugin\s+(install|list)\s+egc@egc\b/.test(source)) {
  ...
  assert.deepStrictEqual(
    offenders,
    [],
    `Deprecated egc@egc install commands must not appear in user-facing docs: ${offenders.join(', ')}`,
  );
});
...
test('README.zh-CN.md version row matches package.json', () => {
  const readme = fs.readFileSync(zhCnReadmePath, 'utf8');
  const match = readme.match(new RegExp(`^\\| \\*\\*版本\\*\\* \\| 插件 \\| 插件 \\| 参考配置 \\| (${semverPattern}) \\|$`, 'm'));
  assert.ok(match, 'Expected README.zh-CN.md version summary row');
  assert.strictEqual(match[1], expectedVersion);
});
