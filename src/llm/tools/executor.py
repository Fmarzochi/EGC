    ...
    def run(self, input: LLMInput) -> LLMOutput:
        ...
        for _ in range(self.max_iterations):
            ...
            for i, result in enumerate(results):
                tool_call = output.tool_calls[i]
                if self.recorder:
                    self.recorder.record("tool_use", {"tool": tool_call.name, "params": tool_call.arguments})
                    self.recorder.record("tool_result", {"tool": tool_call.name, "result": result.content, "is_error": result.is_error})
