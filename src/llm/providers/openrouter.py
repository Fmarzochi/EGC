...
class OpenRouterProvider(OpenAIProvider):
    ...
    def __init__(self, api_key: str | None = None, base_url: str | None = None, **kwargs: Any) -> None:
        ...
        if not key:
            raise AuthenticationError("No OpenRouter API key provided", provider=ProviderType.OPENROUTER)
        ...
        self.client = OpenAI(
            api_key=key,
            base_url=base_url or os.environ.get("OPENROUTER_BASE_URL") or OPENROUTER_BASE_URL,
        ...
        self._models = ModelResolver.model_infos("openrouter") or [
            ModelInfo(name="openrouter/auto", provider=ProviderType.OPENROUTER, supports_tools=True, supports_vision=True, max_tokens=8192, context_window=128000),
        ]
