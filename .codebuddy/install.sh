...
if [ -z "$REPO_ROOT" ]; then
    echo "Error: Cannot locate the EGC repository root."
    echo "This script must be run from within the EGC repository's .codebuddy/ directory."
    exit 1
fi
...
do_install() {
    ...
    MANIFEST="$codebuddy_full_path/.egc-manifest"
    touch "$MANIFEST"
    ...
    echo "  3. Enjoy the EGC workflows!"
    echo ""
