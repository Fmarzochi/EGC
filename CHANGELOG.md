...
- **The DCO check skips merge commits** (#1413): a merge commit joins histories that were already checked and carries no authored content, and the ones GitHub creates through "Update branch" never carry a sign-off anyone can add, so the walk now runs with `--no-merges` the way the reference DCO app does; every other commit still needs its `Signed-off-by`.
