...
- **An engine-only install offers nothing it cannot invoke** (#1467): a bare `egc install` writes no install-state, and the routing read that as an unknown installation and kept every catalog entry available, so `orchestrate_task` on an engine-only Claude Code answered `installation: unknown` with agents and skills the install did not have. A tool named by the environment with no install-state is now a known, empty installation: every matching component lands under `not_installed` with the install hint, in the Node routing and in the Guardian's mirror. The cognitive protocol's review line sends the agent to the `/review-pr` agents only when the prompt library is installed for the tool, and the protocol bumps to v7 so `egc init` and `egc auto-update` rewrite the block on existing installs.
