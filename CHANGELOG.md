...
- **The harness audit scores the capability, not the name of a file** (#1485): the memory-persistence check demanded a `hooks/memory-persistence` directory and the token check a `docs/token-optimization.md`, so a repository that declares its lifecycle hooks in `hooks/hooks.json` or keeps only the guide failed for cosmetics. Each check now accepts either form.
