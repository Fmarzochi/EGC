...
- **The OpenCode plugin index reports the release version** (#1461): `.opencode/index.ts` exported `VERSION` 1.1.13 since that release, because the release script rewrote the line but never committed the file, so every later bump found no match; the file now ships at 1.2.0 and the script commits it with the other propagation files.
