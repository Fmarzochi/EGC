...
function detectTargetMode(rootDir) {
  ...
  if (packageJson?.name === 'everything-gemini') {
    return 'repo';
  }
...
function findPluginInstall(rootDir) {
  ...
  const candidateRoots = [
    ...
    path.join(rootDir, '.gemini', 'plugins', 'marketplaces'),
    homeDir && path.join(homeDir, '.gemini', 'plugins'),
    homeDir && path.join(homeDir, '.gemini', 'plugins', 'marketplaces'),
  ].filter(Boolean);
  const candidates = candidateRoots.flatMap((pluginsDir) =>
    pluginDirs.flatMap((pluginDir) => [
      path.join(pluginsDir, pluginDir, '.gemini-plugin', 'plugin.json'),
...
function getRepoChecks(rootDir) {
  ...
  return [
    ...
    },
    {
      ...
      points: 3,
      scopes: ['repo', 'skills'],
      path: 'skills/workflows/strategic-compact/SKILL.md',
      description: 'Strategic compaction guidance is present',
      pass: fileExists(rootDir, 'skills/workflows/strategic-compact/SKILL.md'),
      fix: 'Add strategic context compaction guidance at skills/workflows/strategic-compact/SKILL.md.',
    },
    {
    ...
    },
    {
      ...
      points: 2,
      scopes: ['repo', 'skills'],
      path: 'skills/ai/continuous-learning-v2/SKILL.md',
      description: 'Continuous learning v2 skill exists',
      pass: fileExists(rootDir, 'skills/ai/continuous-learning-v2/SKILL.md'),
      fix: 'Add skills/ai/continuous-learning-v2/SKILL.md for memory evolution flow.',
    },
    {
      id: 'eval-skill',
      ...
      pass: fileExists(rootDir, 'skills/testing/eval-harness/SKILL.md'),
      fix: 'Add skills/testing/eval-harness/SKILL.md for pass/fail regression evaluation.',
    },
    {
      id: 'eval-commands',
      category: 'Eval Coverage',
      ...
      pass: fileExists(rootDir, 'commands/checkpoint.md') && fileExists(rootDir, 'skills/testing/eval-harness/SKILL.md') && fileExists(rootDir, 'skills/testing/verification-loop/SKILL.md'),
    ...
    },
    {
      ...
      points: 3,
      scopes: ['repo', 'skills'],
      path: 'skills/security/security-review/SKILL.md',
      description: 'Security review skill exists',
      pass: fileExists(rootDir, 'skills/security/security-review/SKILL.md'),
      fix: 'Add skills/security/security-review/SKILL.md for security checklist coverage.',
    },
    {
      id: 'security-agent',
      category: 'Security Guardrails',
    ...
    },
    {
      ...
      points: 4,
      scopes: ['repo', 'skills'],
      path: 'skills/ai/cost-aware-llm-pipeline/SKILL.md',
      description: 'Cost-aware LLM skill exists',
      pass: fileExists(rootDir, 'skills/ai/cost-aware-llm-pipeline/SKILL.md'),
      fix: 'Add skills/ai/cost-aware-llm-pipeline/SKILL.md for budget-aware routing.',
    },
    {
      id: 'cost-doc',
      category: 'Cost Efficiency',
...
function getConsumerChecks(rootDir) {
  ...
  return [
    ...
    },
    {
      ...
      pass: fileExists(rootDir, 'AGENTS.md') || fileExists(rootDir, 'GEMINI.md') || fileExists(rootDir, '.gemini/GEMINI.md'),
    ...
    },
    {
      ...
      pass: fileExists(rootDir, '.github/SECURITY.md') || fileExists(rootDir, '.github/dependabot.yml') || fileExists(rootDir, '.github/codeql.yml'),
...
Audits the current working directory by default and auto-detects EGC repo mode vs consumer-project mode.
`);
