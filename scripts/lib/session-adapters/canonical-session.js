...
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'egc-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  const recordingDir = process.env.EGC_SESSION_RECORDING_DIR || process.env.ECC_SESSION_RECORDING_DIR;
  if (typeof recordingDir === 'string' && recordingDir.length > 0) {
    return path.resolve(recordingDir);
  }
...
function deriveGeminiWorkerId(session) {
...
function normalizeGeminiHistorySession(session, sourceTarget) {
  const metadata = session.metadata || {};
  const workerId = deriveGeminiWorkerId(session);
  const worker = {
    id: workerId,
...
module.exports = {
  ...
  normalizeGeminiHistorySession,
  normalizeDmuxSnapshot,
  persistCanonicalSnapshot,
  validateCanonicalSnapshot,
  // Legacy ECC name kept as a permanent compat alias (never removed).
  normalizeClaudeHistorySession: normalizeGeminiHistorySession
};
