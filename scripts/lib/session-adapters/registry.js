...
const { createGeminiHistoryAdapter } = require('./egc-history');
const { createDmuxTmuxAdapter } = require('./dmux-tmux');

const TARGET_TYPE_TO_ADAPTER_ID = Object.freeze({
  plan: 'dmux-tmux',
  session: 'dmux-tmux',
  'egc-history': 'egc-history',
  'egc-alias': 'egc-history',
  'session-file': 'egc-history'
});
...
function createDefaultAdapters(options = {}) {
  return [
    createGeminiHistoryAdapter(buildDefaultAdapterOptions(options, 'egc-history')),
...
function normalizeStructuredTarget(target, context = {}) {
  ...
  if (type === 'egc-history' || type === 'egc-alias') {
    return {
      target: `egc:${value}`,
      context: nextContext
    };
