...
function getActiveSessionId(provided) {
  return provided || process.env.EGC_SESSION_ID || process.env.ECC_SESSION_ID;
}

function resolveSessionId(rawSessionId) {
  const sid = getActiveSessionId(rawSessionId);
  return sanitizeSessionId(sid || '') || '';
}

function getSessionLeaseFile(context, rawSessionId) {
  const sessionId = resolveSessionId(rawSessionId);
  if (!sessionId) return null;
  return path.join(getSessionLeaseDir(context), `${sessionId}.json`);
}

function writeSessionLease(context, rawSessionId, extra = {}) {
...
function removeSessionLease(context, rawSessionId) {
  const leaseFile = getSessionLeaseFile(context, rawSessionId);
