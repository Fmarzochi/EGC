...
function maybeRunQualityGate(filePath) {
  ...
  const ext = path.extname(filePath).toLowerCase();
  const fix = String(process.env.EGC_QUALITY_GATE_FIX || process.env.ECC_QUALITY_GATE_FIX || '').toLowerCase() === 'true';
  const strict = String(process.env.EGC_QUALITY_GATE_STRICT || process.env.ECC_QUALITY_GATE_STRICT || '').toLowerCase() === 'true';
