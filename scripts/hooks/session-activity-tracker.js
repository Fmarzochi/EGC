 ...
 * ~/.gemini/metrics/tool-usage.jsonl for EGC metric sync.
 */
...
function buildActivityRow(input, env = process.env) {
  ...
  const sessionId = String(env.EGC_SESSION_ID || env.ECC_SESSION_ID || '').trim();
...
function run(rawInput) {
  try {
    ...
    if (row) {
      appendFile(
        path.join(getEGCDir(), 'metrics', METRICS_FILE_NAME),
        `${JSON.stringify(row)}\n`
      );
    }
  } catch {
