...
function getSessionRetentionDays() {
  const raw = process.env.EGC_SESSION_RETENTION_DAYS || process.env.ECC_SESSION_RETENTION_DAYS;
...
function isSessionStartContextDisabled() {
  const raw = String(process.env.EGC_SESSION_START_CONTEXT || process.env.ECC_SESSION_START_CONTEXT || '').trim().toLowerCase();
  return ['0', 'false', 'off', 'none', 'disabled'].includes(raw);
}

function getSessionStartMaxContextChars() {
  const raw = process.env.EGC_SESSION_START_MAX_CHARS || process.env.ECC_SESSION_START_MAX_CHARS;
...
function limitSessionStartContext(additionalContext, maxChars = getSessionStartMaxContextChars()) {
  ...
  const marker = '\n\n[SessionStart truncated context. Set EGC_SESSION_START_MAX_CHARS to raise the cap or EGC_SESSION_START_CONTEXT=off to disable injected context.]';
...
function extractSection(content, headingPattern) {
  ...
  const match = new RegExp(String.raw`^##\s+${headingPattern}\s*\n+([\s\S]+?)(?:\n##\s+|$)`, 'im').exec(source);
...
async function main() {
  ...
  if (explicitContextDisabled) {
    log('[SessionStart] Additional context injection disabled by EGC_SESSION_START_CONTEXT');
