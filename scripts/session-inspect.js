...
function usage() {
  console.log([
    ...
    '  egc:latest        Most recent Gemini session history entry',
    '  egc:<id|alias>    Specific Gemini session or alias',
    '  <session.tmp>        Direct path to a Gemini session file',
    ...
    '  node scripts/session-inspect.js egc:latest',
    '  node scripts/session-inspect.js latest --target-type egc-history',
    '  node scripts/session-inspect.js skills:health',
    '  node scripts/session-inspect.js skills:amendify --skill api-design',
    '  node scripts/session-inspect.js egc:a1b2c3d4 --write /tmp/session.json'
  ].join('\n'));
}
