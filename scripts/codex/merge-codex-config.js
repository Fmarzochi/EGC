...
try {
  ...
  console.error('[egc-codex] Missing dependency: @iarna/toml');
  console.error('[egc-codex] Run: npm install   (from the EGC repo root)');
  process.exit(1);
}
...
function log(message) {
  console.log(`[egc-codex] ${message}`);
}

function warn(message) {
  console.warn(`[egc-codex] WARNING: ${message}`);
}
...
function loadAndParseConfigs(configPath, referencePath) {
  ...
  if (!fs.existsSync(configPath)) {
    console.error(`[egc-codex] Config file not found: ${configPath}`);
    process.exit(1);
  }
