...
List recent EGC sessions from the SQLite state store or inspect a single session
...
function printSessionList(payload) {
  ...
  for (const session of payload.sessions) {
    const sessionUri = `egc://session/${session.id}`;
    console.log(`- ${formatOSC8(session.id, sessionUri)} [${session.harness}/${session.adapterId}] ${session.state}`);
...
function printSessionDetail(payload) {
  const sessionUri = `egc://session/${payload.session.id}`;
  console.log(`Session: ${formatOSC8(payload.session.id, sessionUri)}`);
